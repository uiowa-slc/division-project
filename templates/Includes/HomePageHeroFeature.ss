<div class="hero-article clearfix">
    <% if $Image %>
    	<% if $ExternalLink %>
    		<a href="$ExternalLink" target="_blank"><img src="$Image.URL" alt="An image representing $Title"></a>
    	<% else %>
        	<a href="$AssociatedPage.Link"><img src="$Image.URL" alt="An image representing $Title"></a>
        <% end_if %>
    <% end_if %>
    <h3 class="hero-title">
        <% if $ExternalLink %>
