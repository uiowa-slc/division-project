...
<nav class="sidenav" data-sticky data-margin-top="0" data-anchor="sticky-nav-area" data-sticky-on="large">
	 <% if $Menu(2) %>
		<% with Level(1) %>
			<h2 class="header--small sidenav__section-title"><% if $LinkOrCurrent = "current" %>$MenuTitle<% else %><a href="$Link">$MenuTitle</a><% end_if %></h2>
		<% end_with %>
	...
	<% if $Menu(2) %>
		<ul class="sidenav__menu">
			...
			<% loop $Menu(2) %>
				<li class="sidenav__item sidenav__item--$LinkOrCurrent">
					...
					<% if $LinkOrSection == "section" && $Children %>
						<ul class="sidenav__second-level">
							<% loop Children %>
								<li class="sidenav__item sidenav__item--second-level sidenav__item--$LinkOrCurrent"><a class="sidenav__link" href="<% if $regularLink %>$regularLink<% else %>$Link<% end_if %>">$MenuTitle</a>
									<% if $LinkOrSection = "section" && Children %>
										<ul class="sidenav__third-level">
											<% loop Children %>
												<li class="sidenav__item sidenav__item--third-level sidenav__item--$LinkOrCurrent"><a class="sidenav__link" href="<% if $regularLink %>$regularLink<% else %>$Link<% end_if %>">$MenuTitle</a>
