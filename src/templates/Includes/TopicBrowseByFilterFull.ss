  <div class="topic-browse-by-filter">
      ...
      <% if $AllCats || $AllTags %>
        ...
        <% loop $AllCats.Sort('Title ASC') %>
            <div class="topic-browse-by-filter__item cell small-6 large-4 margin-bottom-1" data-equalizer-watch><a href="$Link" class="button hollow secondary button--flex-full button--skinny"><span class="topicholder-cat-inner">$Title&nbsp;<span style="topicholder-cat-inner__count">({$BlogPosts.Count})</span></span></a></div>
        <% end_loop %>
