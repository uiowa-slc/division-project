  ...
  <div class="grid-container">
      <% if $AllCats %>
        <div class="grid-x grid-padding-x">
          <div class="cell">
               <h2 style="text-transform: uppercase; font-size: 1em; color: #333; ">Categories</h2>
          </div>
        </div>
      
        <div class="grid-x grid-padding-x small-up-2 medium-up-4 large-up-6"  data-equalizer>
          <% loop $AllCats.Sort('Title ASC') %>
            <div class="cell margin-bottom-2" data-equalizer-watch><a href="$Link" class="button large hollow secondary" style="display: flex; height: 100%; border-radius: 3px;"><span style="display: block; margin: auto; ">$Title</span></a></div>
          <% end_loop %>
        </div>
      <% end_if %>
      ...
      <% if $AllTags %>
        <div class="grid-x grid-padding-x">
          <div class="cell">
               <h2 style="text-transform: uppercase; font-size: 1em; color: #333;">Tags</h2>
          </div>
        </div>
      
        <div class="grid-x grid-padding-x small-up-2 medium-up-4 large-up-6"  data-equalizer>
          <% loop $AllTags.Sort('Title ASC') %>
            <div class="cell margin-bottom-2" data-equalizer-watch><a href="$Link" class="button large hollow secondary" style="display: flex; height: 100%;"><span style="display: block; margin: auto;">$Title</span></a></div>
          <% end_loop %>
        </div>
      <% end_if %>
...
<% else %>
      ...
      <div style="background: #eee;">
        <div class="grid-container">
          <div class="grid-x grid-padding-x">
            <div class="cell">
                 <h2>All topics (alphabetical)</h2>
                 <ul style="margin-left: 0; column-count: 3;">
                   <% loop $TopicsByLetter %>
                      <li class="topic-list__item"><h3 tyle="font-weight: 400; font-size: 20px; margin-bottom: 5px; margin-top: 0;">$Letter</h3></li>
                      <% loop $Topics %>
                        <li style="list-style-type: none;"><h3 style="font-weight: normal; font-size: 17px; margin-bottom: 5px; margin-top: 0;"><a style="color: #666;" href="$Link">$Title</a></h3></li>
                      <% end_loop %>
                    <% end_loop %>
