<section class="content-block__container">
	...
	<% if $FacebookPageUrl %>
		...
		<div class="fb-page" data-href="$FacebookPageUrl" data-tabs="$Tabs" <% if $FacebookPluginHeader %> data-small-header="true" <% else %> data-small-header="false" <% end_if %> data-adapt-container-width="true" <% if $FacebookPluginCover %> data-hide-cover="true" <% else %> data-hide-cover="false" <% end_if %> data-width="500" <% if $FacebookPluginFaces %> data-show-facepile="true" <% else %> data-show-facepile="false" <% end_if %>></div>
	<% end_if %>
	</div>
</section>